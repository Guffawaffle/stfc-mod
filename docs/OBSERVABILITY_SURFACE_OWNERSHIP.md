...
| `[advanced.diagnostics].live_query` | `mods/src/config.cc`, `mods/src/defaultconfig.h`, `mods/src/config_sidecar.cc`, `mods/src/patches/parts/live_debug.cc`, `mods/src/patches/frame_tick.cc`, `mods/src/patches/patches.cc` | Canonical enablement gate for the AX/live debug query channel and related frame subscriber wiring | Keep with the general native diagnostics family, not under `[debug]` |
...
- Preserve existing defaults and gates:
  ...
  - `[advanced.diagnostics].mod_impact_monitor = false` by default
  - `[advanced.diagnostics].runtime_trace_report_interval_ms = 5000` by default
  - live query opt-in
