...
The first UI uses Settings > Mod Settings > Shortcuts, grouped into Navigation,
Camera, Actions and Other. It is available in the Windows native settings adapter
...
Only a currently bound row with that target may invoke it. Release restores local
text/interactability overrides and cancels the editor's pending operation.
