...
TEST_CASE("example config does not reintroduce abandoned ghost or manual refresh keys")
{
  ...
  CHECK_FALSE(
      advanced_kirshara_queue->get("try_plan_path_and_engage_target")->value<bool>().value_or(true));
  CHECK(advanced_kirshara_queue->contains("completion_markers"));
  CHECK_FALSE(advanced_kirshara_queue->get("completion_markers")->value<bool>().value_or(true));
