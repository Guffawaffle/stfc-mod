...
namespace
{
  ...
  {
    auto matched_actions =
        compile.index.Match(request.trigger_mode, request.key, request.held_modifiers, request.allow_extra_modifiers);
...
bool ConsumesOriginalKeyEvent(const DispatchCandidate& candidate)
{
  if (candidate.phase != InputPhase::Frame || candidate.trigger_mode != TriggerMode::Down) {
    return false;
  }
...
void PlanDispatchSnapshot(const CompileResult& compile, const InputPhase phase, const ActiveLayers active_layers,
                          const std::span<const DispatchKeyState> key_states, DispatchPlan& plan,
...
DispatchPlan PlanDispatchSnapshot(const CompileResult& compile, const InputPhase phase,
                                  const ActiveLayers active_layers, const std::span<const DispatchKeyState> key_states,
                                  const bool allow_extra_modifiers)
{
  DispatchPlan plan;
...
std::vector<KeyCode> WatchedKeysForActions(const CompileResult& compile, const InputPhase phase,
