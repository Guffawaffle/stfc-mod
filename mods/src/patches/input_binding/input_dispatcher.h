...
[[nodiscard]] bool ConsumesOriginalKeyEvent(const DispatchCandidate& candidate);
...
void PlanDispatchSnapshot(const CompileResult& compile, InputPhase phase, ActiveLayers active_layers,
                          std::span<const DispatchKeyState> key_states, DispatchPlan& plan,
                          bool allow_extra_modifiers = false);
