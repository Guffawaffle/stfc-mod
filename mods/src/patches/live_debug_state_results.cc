...
nlohmann::json TrackerList()
{
  ...
  for (const auto& summary : summaries) {
    ...
    const auto full_name =
        summary.classNamespace.empty() ? summary.className : summary.classNamespace + "." + summary.className;
  ...
  return nlohmann::json{
      {"trackedClassCount", summaries.size()}, {"trackedObjectCount", total_objects}, {"classes", std::move(classes)}};
}
