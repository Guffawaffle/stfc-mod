// Native cargo text snapping; Windows client263 candidate, pinned to verified method extent.
#include "config.h"
...
FieldInfo* Field(Il2CppClass* cls, const char* name, const char* type, size_t bytes)
{
  for (auto* parent = cls; parent; parent = il2cpp_class_get_parent(parent)) {
    ...
    if (field && !(il2cpp_field_get_flags(field) & FIELD_ATTRIBUTE_STATIC) && !field->type->byref
...
bool InstallInstantCargoTextHooks()
{
  ...
  auto* method    = method_contract::Resolve(cls, "SetWidgetData", false, "System.Void", {});
