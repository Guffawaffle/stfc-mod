 ...
 */
MH_HOOK(void, Cursor_SetCursor, void* _this, ptrdiff_t texture, Vector2* hotspot, int cursorMode)
{
#if _WIN32
 ...
 */
MH_HOOK(AppConfig*, Model_LoadConfigs, Model* _this)
{
 ...
 */
MH_HOOK(void, SetActive_hook, void* _this, bool active)
{
 ...
 */
MH_HOOK(bool, IsQueueEnabled, void* _this)
{
...
void InstallTestPatches()
{
  ...
  if (!cursorManager.isValidHelper()) {
    ...
    if (cursorMethod == nullptr) {
      ...
      MH_ATTACH(cursorMethod, Cursor_SetCursor);
    }
  }
  ...
  if (!model.isValidHelper()) {
    ...
    if (load_configs_ptr == nullptr) {
      ...
      MH_ATTACH(load_configs_ptr, Model_LoadConfigs);
    }
  }
  ...
  if (!battle_target_data.isValidHelper()) {
    ...
    if (SetActive == nullptr) {
      ...
      MH_ATTACH(SetActive, SetActive_hook);
    }
  }
  ...
  if (!queue_manager.isValidHelper()) {
    ...
    if (is_queue_unlocked == nullptr) {
      ...
      MH_ATTACH(is_queue_unlocked, IsQueueEnabled);
    }
  }
}
