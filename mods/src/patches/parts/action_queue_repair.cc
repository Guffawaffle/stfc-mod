                                              ...
                                              FleetPlayerData* fleet, int* index)
{
  ...
  if (result) {
    const auto  removed_engaged_target = ActionQueueInstanceIsEngaging(target_instance_before)
                                         || ActionQueueInstanceLastTargetId(target_instance_before) == target_id
                                         || ActionQueueInstanceLastTargetId(target_instance_after) == target_id;
    const auto* reappearance_watch_source = ShouldClearEngagingForRemovedMissingTarget(latest_trigger)
                                                ? "action-queue-remove-after-combat"
                                            : removed_engaged_target ? "action-queue-remove-engaged"
                                                                     : "action-queue-remove";
    live_debug_watch_navigation_fleet_reappearance(reappearance_watch_source, target_id);
  }
