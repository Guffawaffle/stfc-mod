                                             ...
                                             const DecodeOptions& options)
{
  ...
  for (size_t token_index = 0; token_index < battle_log.size(); ++token_index) {
    ...
    if (options.segment_limit == 0 || segments.size() < options.segment_limit) {
      segments.push_back(build_segment_json(battle_log, segment_index, segment_start, token_index, entity_index, options));
    }
    ++segment_index;
  ...
  if (segment_start < battle_log.size()) {
    ...
    if (options.segment_limit == 0 || segments.size() < options.segment_limit) {
      segments.push_back(build_segment_json(battle_log, segment_index, segment_start, battle_log.size() - 1, entity_index, options));
    }
  }
...
[[nodiscard]] nlohmann::json build_parity_status()
{
                              ...
                              uint64_t journal_id_override)
{
  ...
  if (options.include_segments) {
    decoded["segments"] = decode_segments(battle_log, entity_index, options);
  }
