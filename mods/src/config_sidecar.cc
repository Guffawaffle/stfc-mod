...
SidecarConfigParseResult ParseSidecarConfig(const toml::table& config)
{
  ...
  if (const auto* sync = config["sync"].as_table()) {
    if (sync->contains("battlelogs_realtime")) {
      result.diagnostics.push_back(make_diagnostic(
          config_schema::DiagnosticSeverity::Error, "sync.battlelogs_realtime", "sync.battlelogs_realtime",
          "[sync].battlelogs_realtime is invalid. Configure local sidecar battle delivery under "
          "[sidecar.sync].battlelogs_realtime instead."));
    }
    ...
    if (const auto* targets = (*sync)["targets"].as_table()) {
      for (const auto& [target_key, node] : *targets) {
        ...
        if (table.contains("battlelogs_realtime")) {
          result.diagnostics.push_back(make_diagnostic(
              config_schema::DiagnosticSeverity::Error, base_path + ".battlelogs_realtime",
              base_path + ".battlelogs_realtime",
              "[sync.targets.*].battlelogs_realtime is invalid. Local sidecar battle delivery belongs under "
